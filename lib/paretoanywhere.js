...
const BarnaclesMqtt = require('barnacles-mqtt');
...
const RecordSilo = require('record-silo');
...
class ParetoAnywhere {
  ...
  constructor(options) {
    ...
    this.barnacles = createBarnacles(self.barnowl, options);
    this.chimps = createChimps(self.barnowl, self.barnacles, options);
    ...
    this.recordsilo = createRecordSilo(self.app, options);

    initialiseBarnaclesInterfaces(self.barnacles, self.chimps, self.io);
...
/**
 * Initialise and add all the barnacles-x interfaces to barnacles and chimps.
 * @param {Barnacles} The Barnacles instance.
 * @param {Chimps} The Chimps instance.
 * @param {SocketIO} The socket.io instance.
 */
function initialiseBarnaclesInterfaces(barnacles, chimps, io) {
  // Instantiate interfaces with barnacles
  // TODO: used shared MQTT instance
  let barnaclesMqtt = barnacles.addInterface(BarnaclesMqtt, {});
  let barnaclesSocketIO = barnacles.addInterface(BarnaclesSocketIO, { io: io });

  // Add instantiated interfaces to chimps
  chimps.addInterface(barnaclesMqtt);
  chimps.addInterface(barnaclesSocketIO);
}
...
function createBarnowl(app, server, options) {
  ...
  // TODO: use common MQTT client
  barnowl.addListener(BarnowlAxis, {}, BarnowlAxis.MqttListener, {});
  barnowl.addListener(BarnowlZebra, {}, BarnowlZebra.MqttListener, {});
...
function createBarnacles(barnowl, options) {
  ...
  return new Barnacles(options.barnacles);
}
 ...
 */
function createChimps(barnowl, barnacles, options) {
  ...
  return new Chimps(options.chimps);
}
...
/**
 * Create a record-silo instance.
 * @param {Express} app The Express instance.
 * @param {Object} options The configuration options.
 * @return {RecordSilo} The RecordSilo instance.
 */
function createRecordSilo(app, options) {
  if(!options.hasOwnProperty('recordsilo')) {
    options.recordsilo = {};
  }
  options.recordsilo.app = app;

  let recordsilo = new RecordSilo(options.recordsilo);
